...
3. [Fitting a Line to Data (Least Squares)](#3-fitting-a-line-to-data-least-squares)
...
### What it is
A way to measure how wrong your model is. It takes the model's errors and gives you a single number that tells you how bad the predictions are.

### Why it matters
A model that makes predictions will never be perfect. The cost function tells you **how far off** you are so you know which direction to improve.
...
### What it is
An algorithm for finding the lowest point of the cost function — the point where the model's errors are minimized.

### The ball analogy
Imagine placing a ball on a hilly surface. The ball naturally rolls downhill and settles at the lowest point. Gradient descent does the same thing mathematically — it keeps moving in the direction that reduces the cost, step by step, until it reaches the bottom.

### Key ideas
- The **lowest point** of the graph = the best possible settings for your model
- Each step moves the model slightly closer to the bottom
...
### The problem
Given a scatter of data points, how do you find the best line through them?

### The process
1. Start with a line at some angle
2. Measure the **distance** between each data point and the line
3. **Square** each distance (so negatives and positives don't cancel out)
4. **Sum** all the squared distances → this is your Sum of Squares
5. Rotate the line and repeat
6. The line with the **lowest Sum of Squares** is the best fit

### Why we square the distances
- Prevents negative errors cancelling out positive errors
- Penalizes large errors more than small ones
- Gives a smooth curve you can minimize mathematically
...
### Intuition
`a` tells you the **relationship strength and direction** between x and y. If `a = 3`, every 1 unit increase in x predicts a 3 unit increase in y.
...
### What it is
A measure of how well your model explains the data — and how much better it is compared to just using the mean.

### Why R² and not R
R² = 0.7 is **not** just slightly better than R² = 0.5. It is actually twice as good at explaining the variation in the data. R alone would not show you that difference clearly. R² makes the comparison meaningful.
...
This is the full workflow for building and evaluating a linear regression model:

### Step 1 — Fit the line using Least Squares
Find the line that minimizes the sum of squared distances between points and the line.

### Step 2 — Calculate R²
Measure how much better your line is compared to just using the mean. This tells you if your model actually learned anything useful.

### Step 3 — Calculate the P-value of R²
Check whether your R² result is statistically significant or could have happened by chance.
